...
/** Lightweight syntax highlighter for the docs Code panes — produces `.tok-*` spans matching
 *  the bundle's palette (keyword=primary, string=signal, number=warn, comment=ink-3 italic,
 *  function=warm orange/brown, punctuation=ink-2). No deps. */
...
function tokenizeTs(code: string): Token[] {
  ...
  while (i < len) {
    ...
    // string (no escape handling — good enough for code snippets)
    if (c === '"' || c === "'" || c === "`") {
      const quote = c;
      let j = i + 1;
    ...
    if (/[A-Za-z_$]/.test(c)) {
      ...
      // skip whitespace to peek for `(` → function call
      let k = j;
...
/** Render a code string as syntax-highlighted JSX. */
