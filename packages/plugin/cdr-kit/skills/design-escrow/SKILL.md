---
name: design-escrow
description: Design a Story CDR ConditionalEscrowCondition vault — buyer-pays-then-confirms-delivery flow with optional arbiter dispute path and seller-side timeout claim. Use this skill whenever the user mentions escrow, data marketplace, pay-per-read with delivery confirmation, dispute resolution for data sales, refunds for digital downloads, holding payment until delivery, or asks how to sell data with a 2-step settlement, how to add a refund path to a vault, what happens if the buyer goes silent after paying, or how to gate a vault behind a payment that releases on confirmation.
license: MIT
---
