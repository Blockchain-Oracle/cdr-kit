...
description: Use the cdr-kit agent-as-publisher one-shot to ship a Story-IP-registered, license-gated CDR vault in a single call. Use this skill whenever the user mentions publishing data with Story Protocol, monetizing data via PIL license tokens, agent-as-publisher patterns, registering IP and gating reads in one tx, the cdr-kit publish() flow, license-gated vaults, commercial-use vs commercial-remix vs non-commercial PIL flavors, or asks how to sell encrypted data with Story IP licensing, how PIL terms flow through to CDR reads, what the SPG NFT contract is for, or how to combine @cdr-kit/story with @cdr-kit/agent.
license: MIT
---
