<!-- Source-of-truth lives at packages/plugin/cdr-kit/skills/design-multisig-condition/SKILL.md — keep these synchronized when editing. -->
---
name: design-multisig-condition
description: Design a Story CDR MultiSigCondition vault (N-of-M EIP-712 threshold reads, signer rotation, off-chain signature collection). Use this skill when (1) the user wants to gate a CDR vault behind multiple signers, board approval, or a DAO threshold; (2) the user asks how off-chain EIP-712 sigs compose without on-chain approve(); (3) the user wants to rotate signers or invalidate in-flight sigs via epoch bump; (4) the user is integrating Safe / Gnosis or wiring the MultiSigSigner / MultiSigApprovalTracker React components.
license: MIT
---
